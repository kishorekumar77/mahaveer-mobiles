...
// ── Force Google DNS — fixes ECONNREFUSED on restricted networks ──
...
// Build direct connection string using resolved IPs
// This bypasses SRV DNS lookup which is blocked on some networks
async function connectMongo() {
  // First try normal SRV connection
  try {
    ...
    console.log('⚠️  SRV connection failed, trying direct IP connection...');
  }

  // Second try: resolve SRV manually with Google DNS then connect directly
  try {
    const dnsPromises = require('dns').promises;
    const resolver = new (require('dns').Resolver)();
    resolver.setServers(['8.8.8.8', '8.8.4.4']);

    const srvRecords = await resolver.resolveSrv('_mongodb._tcp.mahaveer.x409he1.mongodb.net');
    const hosts = srvRecords.map(r => `${r.name}:${r.port}`).join(',');
    const directURI = `mongodb://${encodeURIComponent('mahaveer')}:${encodeURIComponent('mahaveer@123')}@${hosts}/mahaveer?ssl=true&replicaSet=atlas-14ib3t-shard-0&authSource=admin&retryWrites=true&w=majority`;
    ...
    console.log('✅ MongoDB connected via direct IP!');
    seedDatabase();
    return;
  } catch (e2) {
    console.error('❌ MongoDB connection failed:', e2.message);
    console.log('');
    console.log('⚠️  Running in OFFLINE mode — using local JSON files');
    console.log('   Website still works at http://localhost:' + PORT);
    console.log('');
    console.log('   SOLUTION: Your network blocks port 27017.');
    console.log('   Please use your MOBILE HOTSPOT to connect.');
  }
}
